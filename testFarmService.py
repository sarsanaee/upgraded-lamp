...
admin = Admin(app, name='farm_web_service_admin', template_mode='bootstrap3')
#admin.add_view(ModelView(User,db_session))
#admin.add_view(ModelView(Level, db_session))
#admin.add_view(ModelView(Transaction, db_session))
#admin.add_view(ModelView(Xmlbase, db_session))
#admin.add_view(ModelView(Giftcards, db_session))
...
# Create in-memory database
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///testdbdbdb.sqlite'
app.config['SQLALCHEMY_ECHO'] = True
db = SQLAlchemy(app)
...
def valid_giftcard():
    ...
    if gift_ret and gift_ret.validity:
        gift_ret.validity = 0
        db_session.commit()
        response = jsonify({"status": "200", 'count': gift_ret.count})
        response.status_code = 200
        return response
    abort(404)
