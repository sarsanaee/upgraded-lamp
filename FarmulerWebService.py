...
from models import User, Level, Transaction, Xmlbase, Giftcards, AdminUsers, Store, GameDb, Special_Packages
...
admin = Admin(app, name='farm_web_service', index_view=MyAdminIndexView(), base_template='templates/my_master.html')
...
def validation_v1():
    ...
    cafebazaar_send_validation_request.delay(product_id, purchase_token, request.json.get("id"))
...
def cafebazaar_send_validation_request(product_id, purchase_token, id):
    ...
    result = return_json.has_key('error')

    is_repeated_token = db_session.query(Transaction).filter_by(token=purchase_token).all()

    if result and not is_repeated_token:
        product_ids = db_session.query(Store.product_id).all()
        special_packages_product_ids = db_session.query(Special_Packages.product_id).all()
        print(id)
        user_db = GameDb.query.filter_by(user_id=id).first()
        '''
        for i in product_ids:
            if product_id in i:
                store = Store.query.filter_by(product_id=product_id).first()
                user_db.Diamonds = store.diamond + user_db.Diamonds
                transaction = Transaction(id, store.discount, store.diamond, store.price, purchase_token, product_id)
                db_session.add(transaction)
                break
        for i in special_packages_product_ids:
            if product_id in i:
                ...
                user_db.Diamonds = special_packages.diamond + user_db.Diamonds
                user_db.Coins = special_packages.coin + user_db.Coins
                transaction = Transaction(id, special_packages.discount, special_packages.diamond, special_packages.price, purchase_token, product_id)
                db_session.add(transaction)
                break

        db_session.commit()
        '''
    print(result, is_repeated_token)
    return result and is_repeated_token
